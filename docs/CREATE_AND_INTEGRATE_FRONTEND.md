...
Hub listens on `PORT` (default **3000**). Health: `GET /xbot/v1/api/hub/health` → `{ "status": "ok" }`. All Hub routes use **`/xbot/v1/api/hub`**.
...
| `X_REDIRECT_URI` | Stays on **Hub** (`/xbot/v1/api/hub/oauth/x/callback`) |
...
| Endpoint | Auth | Body |
|----------|------|------|
| `POST /xbot/v1/api/hub/auth/register` | None | `{ "email", "password" }` — password min 8 chars |
| `POST /xbot/v1/api/hub/auth/login` | None | `{ "email", "password" }` |
| `GET /xbot/v1/api/hub/auth/me` | Bearer | — |
...
export async function hubFetch<T>(
  ...
  const res = await fetch(`${base}/xbot/v1/api/hub${path}`, {
    ...rest,
    headers: {
...
2. `POST /xbot/v1/api/hub/orgs` with `{ "name": "Acme" }` (optional `slug`).
3. `GET /xbot/v1/api/hub/orgs` → each org includes `role` (`owner` | `admin` | `member`).
...
1. `POST /xbot/v1/api/hub/orgs/:orgId/invites` — optional `{ "expiresInHours": 24, "maxUses": 5 }` (defaults: 168h, unlimited).
2. Response: `inviteToken`, `inviteUrl` = `{HUB_PUBLIC_BASE_URL}/xbot/v1/api/hub/oauth/x/start?invite=<token>`.
...
1. `GET /xbot/v1/api/hub/invites/:token` (no auth) → `{ orgName, expired, revoked, maxUsesReached, ... }`.
2. If valid, navigate to OAuth start (full page redirect, not `fetch`):

   ```
   {HUB_PUBLIC_BASE_URL}/xbot/v1/api/hub/oauth/x/start?invite={token}
   ```
...
`GET /xbot/v1/api/hub/orgs/:orgId/members` → `{ userId, email, role, joinedAt }[]`
...
`POST /xbot/v1/api/hub/orgs/:orgId/campaigns`
...
`GET /xbot/v1/api/hub/orgs/:orgId/campaigns/:campaignId/status`
...
Base: `{HUB_ORIGIN}/xbot/v1/api/hub`
...
| Path | Role |
|------|------|
| `apps/hub/src/main.ts` | Global prefix `xbot/v1/api/hub`, CORS |
